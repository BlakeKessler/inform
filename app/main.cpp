...
//random proof generation parameters
//the comments at the end of each line are regexes for the CLI flag for that parameter
...
//!NOTE: inPath and allowContConc are not currently supported


//randomly generate the proofs according to the parameters
...
//parse CLI inputs and extract parameters
bool parseInputs(int argc, char** argv) {
   ...
   for (int i = 1; i < argc; ++i) {
      ...
      //    continue;
      // }
      if (arg.size() == 2 && arg[1] == 'A') { //use ASCII encoding (default encoding is the default unicode encoding of the compiler used to compile the project)
         inform::NOT = mcsl::FMT("~");
         inform::OR = mcsl::FMT("|");
         inform::AND = mcsl::FMT("&");
...
void help(mcsl::str_slice exePath) {
   ...
   mcsl::printf(FMT("\033[4m-t\033[24m: the maximum number of terms in each premise\n"));
   mcsl::printf(FMT("\033[4m-v\033[24m: the number of variables\n"));
   mcsl::printf(FMT("\033[4m-s\033[24m: increasing this number will make each term sparser\n"));
...
int main(int argc, char** argv) {
   #ifdef NDEBUG
   mcsl::srand(std::time(nullptr)); //set an unpredictable RNG seed in release builds
   #endif

   //parse inputs
   if (argc == 1 || (argc == 2 && (FMT(argv[1]) == FMT("-h") || FMT(argv[1]) == FMT("--help")))) { //print help menu and exit the program
      help(FMT(argv[0]));
      return EXIT_SUCCESS;
   }
